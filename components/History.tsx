...
const History: React.FC<HistoryProps> = ({ meds, logs }) => {
  ...
  return (
    <div className="p-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {/* Statistics Card */}
      <div className="bg-slate-900 rounded-3xl p-5 text-white mb-8 shadow-lg shadow-slate-200 dark:shadow-slate-900">
      ...
      {/* Filters Section */}
      <div className="bg-white dark:bg-slate-700 border border-slate-100 dark:border-slate-600 rounded-3xl p-4 mb-6 shadow-sm space-y-4">
        ...
        <div className="space-y-3">
          <select
            className="w-full px-4 py-2 bg-slate-50 dark:bg-slate-600 border border-slate-100 dark:border-slate-500 rounded-xl text-sm text-slate-800 dark:text-white outline-none focus:ring-2 focus:ring-blue-500"
            value={filterMedId}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="relative">
              ...
              <input
                type="date"
                className="w-full pl-9 pr-3 py-2 bg-slate-50 dark:bg-slate-600 border border-slate-100 dark:border-slate-500 rounded-xl text-xs text-slate-800 dark:text-white outline-none focus:ring-2 focus:ring-blue-500"
                value={startDate}
      ...
      <div className="space-y-3">
        {filteredLogs.length === 0 ? (
          ...
          filteredLogs.map(log => {
            ...
            return (
              <div key={log.id} className="flex items-center gap-4 p-4 bg-white dark:bg-slate-700 border border-slate-50 dark:border-slate-600 rounded-2xl shadow-sm">
                <div className={`w-10 h-10 ${med?.color || 'bg-slate-200 dark:bg-slate-600'} rounded-xl flex items-center justify-center text-white shrink-0`}>
                  <HistoryIcon size={18} />
                </div>
                ...
                <div className="text-right">
                  <span className="text-[10px] bg-slate-50 dark:bg-slate-600 text-slate-500 dark:text-slate-300 px-2 py-1 rounded-md font-bold uppercase tracking-tighter">
                    Przyjęto
                  </span>
                </div>
              </div>
